...
struct EndlessGameMode: View {
    ...
    var body: some View {
        ...
        .sheet(
            ...
            onDismiss: {
                ...
                if isPlayerTurn && !roundOver && !showARThrow {
                    startPlayerTurn()
                }
            }
        ) {
        ...
        VStack {
            ...
            ZStack {
                Image("continue-button")
                    .resizable()
                    .scaledToFit()
                    .frame(height: roundOver ? 72 : 48)
                
                Text(turnLabel)
                    .font(.system(size: roundOver ? 32 : 20, weight: .bold, design: .serif))
                    .foregroundColor(.black)
            }
            .padding(.top, 90)
            .padding(.bottom, 36)
            
            EndlessShagaiPlayground(
                round: trackLength,
            ...
            HStack {
                EndlessShagaiPlaygroundScore(
                    playerPosition: playerPosition,
                    botPosition: botPosition,
                    trackLength: trackLength
                )
                EndlessPlaygroundEnemyThrow(
                    horse: botHorse,
                    goat: botGoat,
                    sheep: botSheep,
                    camel: botCamel
    ...
    private func startPlayerTurn() {
        ...
        turnLabel = "Your turn!"
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            guard !roundOver, !showTutorial else { return }
    ...
    private func stepPlayer(remaining: Int) {
        if remaining > 0 && playerPosition < trackLength - 1 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                ...
                self.stepBot(remaining: botResult.horse)
            }
        }
    }
    
    private func stepBot(remaining: Int) {
        if remaining > 0 && botPosition < trackLength - 1 {
            botPosition += 1
            
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
                self.stepBot(remaining: remaining - 1)
            }
        } else {
            if botPosition >= trackLength - 1 {
                roundOver = true
...
struct EndlessShagaiPlayground: View {
    var round: Int
    var playerPosition: Int = 0
    var botPosition: Int = 0
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Image("enemy-shagai")
                ...
                .scaledToFit()
                .frame(height: 56)
                .offset(x: CGFloat(min(botPosition, max(round - 1, 0))) * cellWidth)
                .animation(.easeInOut(duration: 0.6), value: botPosition)
        }
    }
}

struct EndlessShagaiPlaygroundScore: View {
    var playerPosition: Int = 0
    var botPosition: Int = 0
    var trackLength: Int = 10
                ...
                HStack {
                    ...
                    HStack {
                        Image("enemy-shagai")
                            ...
                            .scaledToFit()
                            .frame(height: 58)
                        Text("\(botPosition + 1)/\(trackLength)")
                            .font(.system(size: 24, weight: .bold, design: .serif))
                            .foregroundColor(.black)
                    }
                }
...
struct EndlessPlaygroundEnemyThrow: View {
    var horse: Int = 0
    var goat: Int = 0
    var sheep: Int = 0
    var camel: Int = 0
    ...
    var body: some View {
        VStack {
            ...
            ZStack {
                ...
                HStack {
                    BotThrowItem(imageName: "shagai-horse", count: "\(horse)")
                    BotThrowItem(imageName: "shagai-goat", count: "\(goat)")
                    BotThrowItem(imageName: "shagai-sheep", count: "\(sheep)")
                    BotThrowItem(imageName: "shagai-camel", count: "\(camel)")
                }
            }
        }
    }
